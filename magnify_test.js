...
var MAGNIFIER = function (alertOnErrors) {
    ...
    function mouseTrack(evt) {
      ...
      var data = getImagePairData(x, y);
        ...
        if (data){
          if (evt.currentTarget.magnifyEnable){
            console.log("enable and data in");
            updateMagnifiedImage(x, y, data);
          }
          else {
            console.log("disable and data in");
            hideMagnifiedImage(data.large.image);
          }
        }
    ...
    function initListeners() {
      if (document.addEventListener) {
        document.magnifyEnable = false;
        console.log(document.magnifyEnable);
        var magnifyEnable = document.querySelector("input[name=mag-switch]");
        magnifyEnable.addEventListener('change', function() {
          if (this.checked){
            document.magnifyEnable = true;
          } else{
            document.magnifyEnable = false;
          }
